...
struct common_speculative_impl {
    ...
    size_t n_call_process = 0; // number of times this implementation was called for batch processing.
    ...
    size_t n_process_tokens = 0; // number of tokens processed by this implementation.

    // TODO: track performance of most recent calls
    const bool gen_perf = true; // whether to generate performance stats.

    int64_t t_begin_us  = 0; // total time spent in refresh of this implementation in microseconds.
    int64_t t_process_us = 0; // total time spent in processing target batches in microseconds.
...
bool common_speculative_process(common_speculative * spec, const llama_batch & batch) {
    ...
    for (auto & impl : spec->impls) {
        ...
        impl->n_call_process++;
        impl->n_process_tokens += batch.n_tokens;
    }
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        ...
        LOG_INF("statistics %s: #calls(b,p,g,a) = %zu %zu %zu %zu, #process tokens = %zu, #gen drafts = %zu, #acc drafts = %zu, #gen tokens = %zu, #acc tokens = %zu%s\n",
                common_speculative_type_to_str(impl->type).c_str(),
                impl->n_call_begin, impl->n_call_process, impl->n_call_draft, impl->n_call_accept,
                impl->n_process_tokens,
                impl->n_gen_drafts,
                impl->n_acc_drafts,
                impl->n_gen_tokens,
                impl->n_acc_tokens,
