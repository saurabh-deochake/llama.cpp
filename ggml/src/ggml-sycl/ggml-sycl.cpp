...
GGML_BACKEND_API void * ggml_backend_sycl_device_malloc(size_t size, ggml_backend_t backend) try {
    GGML_SYCL_DEBUG("[SYCL] call %s size=%zu", __func__, size);
    ggml_backend_sycl_context * sycl_ctx = (ggml_backend_sycl_context *)backend->context;
    queue_ptr stream = sycl_ctx->stream(sycl_ctx->device, 0);
    void * ptr = sycl::malloc_device(size, *stream);
    GGML_SYCL_DEBUG(" -> %p\n", ptr);
    return ptr;
}
catch (sycl::exception const &exc) {
...
GGML_BACKEND_API void ggml_backend_sycl_device_free(void * ptr, ggml_backend_t backend) try {
    GGML_SYCL_DEBUG("[SYCL] call %s ptr=%p", __func__, ptr);
    ggml_backend_sycl_context * sycl_ctx = (ggml_backend_sycl_context *)backend->context;
    queue_ptr stream = sycl_ctx->stream(sycl_ctx->device, 0);
    sycl::free(ptr, *stream);
}
...
GGML_BACKEND_API void ggml_backend_sycl_device_memcpy(void * dst, const void * src, size_t size, ggml_backend_t backend) try {
    GGML_SYCL_DEBUG("[SYCL] call %s dst=%p src=%p size=%zu", __func__, dst, src, size);
    ggml_backend_sycl_context * sycl_ctx = (ggml_backend_sycl_context *)backend->context;
    queue_ptr stream = sycl_ctx->stream(sycl_ctx->device, 0);
    SYCL_CHECK(CHECK_TRY_ERROR((stream)->memcpy(dst, (const void *)src, size)));
}
catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
